      ...
      <div class="box-content">
        ...
        <table class="table">
          ...
          <c:forEach items="${list}" var="e" varStatus="st">
            <tr>
              <td>${st.index + 1}</td>
              <td><a href="javascript:void (0);"><img class="img-rounded" width="50" height="50" src="${e.picAddress}" onclick="ImgShow(event)" ></a></td>
              <td>${e.schoolName}</td>
              <td>${e.desc}</td>

              <td>
                <button class="btn btn-primary" type="button" onclick="deleteViewpager('${e.id}')">删除</button>
              </td>
            </tr>
          </c:forEach>
          </tbody>
  ...
  function deleteViewpager(_id){
    ...
    $.ajax({
      ...
      success: function(_data) {
        ...
        if(data.success){
          alert("删除成功");
            window.location.href="#module=/viewpager/list&_t="+new Date().getTime();
        }else{
          var _case = {1:"删除失败"};
