        ...
        <table class="table">
          ...
          <c:forEach items="${list}" var="e" varStatus="st">
            <tr>
              ...
              <td>
                <a class="btn btn-default btn-sm" href="#module=/toSellerGoodsSetting&empId=${e.empId}&schoolId=${e.schoolId}" role="button">设置</a>
                <button class="btn btn-primary" type="button" onclick="deleteSeller('${e.empId}','${e.contractId}')">删除</button>
              </td>
            </tr>
          </c:forEach>
          </tbody>
...
<script type="text/javascript">
  function deleteSeller(_empid, _contractId){
    if(!confirm("确定要删除该商家？")){
      return;
    }
    $.ajax({
      ...
      type: "POST",
      url:"/deleteSeller.do",
      data:{"empId":_empid, "contractId":_contractId},
      async: false,
      success: function(_data) {
        var data = $.parseJSON(_data);
        if(data.success){
          alert("删除成功");
          window.location.href="#module=listSeller&_time="+new Date().getTime();
        }else{
          var _case = {1:"删除失败"};
  ...
  function searchIndex(e){
    ...
    if(_index <= ${page.pageCount} && _index >= 1){
      window.location.href="#module=listSeller&page="+_index+"&size="+size;
    }else{
  ...
  function nextPage(_page) {
    ...
    if ((page <= ${page.pageCount} && page >= 1)) {
      window.location.href="#module=listSeller&page="+page+"&size="+size;
    } else {
      alert("请输入1-${page.pageCount}的页码数");
    }
  }
